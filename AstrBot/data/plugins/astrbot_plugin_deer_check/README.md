...
- `🦌年历`
  ...
  - **功能**: 查看本年度的完整打卡日历。
  - **回复**: 一张显示本年度（到当前月份）的打卡日历图。
  - **支持年份**: `🦌年历 2024` 查看指定年份年历。
- `🦌报告`
...
- `🦌报告 月份`
  ...
  - **功能**: 分析指定月份的打卡数据并生成报告。
  - **回复**: 一张包含打卡统计和分析的图片，例如 `🦌分析 11` 分析11月的数据。
- `🦌报告 年份`
  
  - **功能**: 分析指定年份的打卡数据并生成报告。
  - **回复**: 一张包含打卡统计和分析的图片，例如 `🦌分析 2025` 分析2025年的数据。
- `🦌月历 月份`
  
  - **功能**: 查看指定月份的打卡日历。
  - **回复**: 一张指定月份的打卡日历图，例如 `🦌月历 11` 查看11月的日历。
- `🦌排行`
...
- `🦌生涯`
  ...
  - **功能**: 生成个人生涯数据分析报告。
  - **回复**: 一张包含生涯统计、称号、最长记录等数据的图片。
- `🦌补签`
  
  - **命令格式**: `🦌补签+日期(如:1)+次数(如:18)`
    - `🦌补签 1 18`
    - `🦌补签 1` (默认补签1次)
  - **功能**: 补签当月中某天的`🦌`数，如果当天已有记录，会累加次数。
  - **回复**: 一张补签后的本月打卡日历图。
- `🦌撤销`
  
  - **命令格式**: `🦌撤销+日期(如:1)+次数(如:1)`
    - `🦌撤销 1 1`
    - `🦌撤销 1` (默认撤销1次)
  - **功能**: 撤销当月中某天的`🦌`数。
  - **回复**: 一张撤销后的本月打卡日历图。
...
* 如果您在使用中遇到错误或有算法改进建议，欢迎提交 [Issue](https://github.com/DITF16/astrbot_plugin_deer_check/issues )。
