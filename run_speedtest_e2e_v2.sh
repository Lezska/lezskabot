...
cat > /tmp/speedtest_e2e2.py << 'PYEOF'
"""Real e2e speedtest v2: shared conn_mgr across 4 critical repos + P3 transaction."""
import sys, os, shutil, time, asyncio, sqlite3, types

# 1. Setup env
SRC = "/root/lezskabot/AstrBot/data/fish.db"
DST = "/tmp/fish_e2e2.db"
...
# 4. SHARED conn_mgr (A1) for all 4 go_fish critical repos
...
# 5. Build real plugin stack with shared conn_mgr
...
item_tpl_repo = SqliteItemTemplateRepository(DST)  # not in go_fish critical path
...
print(f"fishing_service._conn_mgr is conn_mgr: {fishing_service._conn_mgr is conn_mgr}")
...
# 9. Warmup
print("warmup (3 fish via effect)...")
...
reset()
print(f"\n[BASELINE] running {N}x go_fish (P0+P1 path, no transaction)...")
...
reset()
print(f"\n[P3] running effect.apply(user, item, qty={N}) (with shared conn_mgr + transaction)...")
t0 = time.perf_counter()
...
print(f"  result.message first line: {res.get('message','').split(chr(10))[0]}")
...
print(f"\nResults:")
print(f"  BASELINE (P0+P1, no txn): {baseline:.2f}s")
print(f"  P3 (shared conn_mgr + txn): {p3:.2f}s")
if p3 > 0:
...
python /tmp/speedtest_e2e2.py 2>&1 | tail -50
