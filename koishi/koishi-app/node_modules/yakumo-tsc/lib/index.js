...
__name(prepareBuild, "prepareBuild");
function getFiles(meta, outDir) {
  function addExport(pattern) {
    if (!pattern) return;
  ...
  function addConditionalExport(pattern) {
    if (!pattern) return;
...
__name(bundleNodes, "bundleNodes");
var inject = ["yakumo"];
function apply(ctx) {
  ctx.register("tsc", async () => {
    ...
    for (const path of paths) {
      ...
      try {
        ...
        const bundle2 = config.compilerOptions?.outFile ? true : config.compilerOptions?.outDir ? false : files.length === 1 && !!meta.exports;
        nodes[meta.name] = { config, bundle: bundle2, path, meta, prev: [], next: /* @__PURE__ */ new Set() };
      } catch (error) {
