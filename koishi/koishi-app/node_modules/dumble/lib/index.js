...
var DependencyType = ["dependencies", "devDependencies", "peerDependencies", "optionalDependencies"];
var ignored = [
...
var displayError = display(kleur.red("error:"));
var displayWarning = display(kleur.yellow("warning:"));
...
var externalPlugin = /* @__PURE__ */ __name(({ manifest, exports }) => ({
  name: "external library",
  setup(build2) {
    const { entryPoints, platform, format } = build2.initialOptions;
    const currentEntry = Object.values(entryPoints)[0];
    build2.onResolve({ filter: /^[@\w].+$/ }, (args) => {
      if (isAbsolute(args.path)) return null;
      if (isBuiltin(args.path)) return { external: true };
      const name = args.path.startsWith("@") ? args.path.split("/", 2).join("/") : args.path.split("/", 1)[0];
    ...
    build2.onResolve({ filter: /^\./, namespace: "file" }, async (args) => {
      const { path } = await build2.resolve(args.path, {
        namespace: "internal",
    ...
    build2.onLoad({ filter: /\.d\.ts$/ }, async (args) => {
      const contents = await fs.readFile(args.path);
      return { loader: "copy", contents };
    });
  }
}), "externalPlugin");
var yamlPlugin = /* @__PURE__ */ __name((options = {}) => ({
  name: "yaml",
  setup(build2) {
    build2.initialOptions.resolveExtensions.push(".yml", ".yaml");
    build2.onLoad({ filter: /\.ya?ml$/ }, async ({ path }) => {
...
var hashbangPlugin = /* @__PURE__ */ __name((binaries) => ({
  name: "hashbang",
  setup(build2) {
    build2.onLoad({ filter: /\.tsx?$/ }, async ({ path }) => {
      if (!binaries.includes(path)) return null;
      let contents = await fs.readFile(path, "utf8");
      if (!contents.startsWith("#!")) {
        contents = "#!/usr/bin/env node\n" + contents;
      }
      return { contents, loader: extname(path).slice(1) };
    });
  }
}), "hashbangPlugin");
async function dumble(cwd, manifest, tsconfig, config = {}) {
  ...
  const outFiles = /* @__PURE__ */ new Set();
  const binaries = [];
  const resolveCache = /* @__PURE__ */ Object.create(null);
  async function resolvePattern(pattern) {
    ...
    return [outExt, await globby(pattern, { cwd: outbase })];
  }
  __name(resolvePattern, "resolvePattern");
  async function addExport(pattern, preset2, prefix = "", isBinary = false) {
    if (!pattern) return;
    ...
    for (const target of targets) {
      ...
      const outFile2 = join(outdir, entry + outExt);
      if (outFiles.has(outFile2)) return;
      outFiles.add(outFile2);
      if (!preset2.platform) {
        ;
        (exports[srcFile] ||= {}).types = `${manifest.name}/${prefix}`;
      } else {
        ;
        (exports[srcFile] ||= {})[preset2.platform] = outFile2;
      }
      matrix.push({
        absWorkingDir: cwd,
        outdir,
  ...
  __name(addExport, "addExport");
  const tasks = [];
  function addConditionalExport(pattern, preset2, prefix = "") {
    if (typeof pattern === "string") {
      tasks.push(addExport(pattern, preset2, prefix));
      return;
    }
    for (const key in pattern) {
      if (key === "require") {
        addConditionalExport(pattern[key], { ...preset2, format: "cjs" }, prefix);
      } else if (key === "import") {
        addConditionalExport(pattern[key], { ...preset2, format: "esm" }, prefix);
      } else if (["browser", "node"].includes(key)) {
        addConditionalExport(pattern[key], { ...preset2, platform: key }, prefix);
      } else if (["types", "typings"].includes(key)) {
        addConditionalExport(pattern[key], { ...preset2, platform: void 0 }, prefix);
      } else {
        addConditionalExport(pattern[key], preset2, key.startsWith(".") ? join(prefix, key) : prefix);
      }
    }
  }
  __name(addConditionalExport, "addConditionalExport");
  const preset = {
    platform: "node",
  ...
  const build2 = config.build ?? ((options, callback) => callback(options));
  await Promise.all(matrix.map(async (options) => {
    try {
      await build2(options, bundle);
    } catch (error) {
      console.error(error);
    }
  }));
}
__name(dumble, "dumble");
var index_default = dumble;
export {
  DependencyType,
