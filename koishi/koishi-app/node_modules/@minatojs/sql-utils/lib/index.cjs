...
var Builder = class {
  constructor(driver, tables) {
    ...
    this.evalOperators = {
      ...
      $log: /* @__PURE__ */ __name((args) => `log(${args.filter((x) => !(0, import_cosmokit.isNullable)(x)).map((arg) => this.parseEval(arg)).reverse().join(", ")})`, "$log"),
  ...
  get(sel, inline = false, group = false, addref = true) {
    ...
    if (inline && !args[0].fields && !suffix && (typeof table === "string" || import_minato.Selection.is(table))) {
  ...
  escapePrimitive(value, type) {
    if ((0, import_cosmokit.isNullable)(value)) return "NULL";
    switch (typeof value) {
      case "boolean":
      case "number":
      case "bigint":
