...
import { Driver as Driver2, Eval, executeUpdate, Field as Field2, getCell, hasSubquery, isEvalExpr as isEvalExpr2, z } from "minato";
...
var SQLiteDriver = class extends Driver2 {
  ...
  async set(sel, update) {
    ...
    if (query.$expr || hasSubquery(sel.query) || Object.values(update).some((x) => hasSubquery(x))) {
      ...
      sel2.ref = sel.ref;
      const project = mapValues(update, (value, key) => () => isEvalExpr2(value) ? value : Eval.literal(value, model.getType(key)));
