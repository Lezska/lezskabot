...
var import_sql = __toESM(require("@minatojs/sql.js"), 1);
...
var SQLiteBuilder = class extends import_sql_utils.Builder {
  constructor(driver, tables) {
    ...
    this.evalOperators.$number = (arg) => {
      ...
      const res = import_minato.Field.date.includes(type.type) ? `cast(${value} / 1000 as integer)` : `cast(${this.parseEval(arg)} as double)`;
    ...
    this.transformers["binary"] = {
      ...
      load: /* @__PURE__ */ __name((value) => (0, import_cosmokit.isNullable)(value) || typeof value === "object" ? value : import_cosmokit.Binary.fromHex(value), "load"),
      dump: /* @__PURE__ */ __name((value) => (0, import_cosmokit.isNullable)(value) || typeof value === "string" ? value : import_cosmokit.Binary.toHex(value), "dump")
    };
  }
  static {
  ...
  escapePrimitive(value, type) {
    ...
    else if (import_cosmokit.Binary.is(value)) return `X'${import_cosmokit.Binary.toHex(value)}'`;
    else if (import_cosmokit.Binary.isSource(value)) return `X'${import_cosmokit.Binary.toHex(import_cosmokit.Binary.fromSource(value))}'`;
  ...
  createMemberEval(rawKey, value, notStr = "") {
    ...
    } else if (import_minato.Type.fromTerm(value)?.type === "list") {
      const res = this.listContains(this.parseEval(value), key);
      return notStr ? this.logicalNot(res) : res;
    } else {
      const res = this.jsonContains(this.parseEval(value, false), (0, import_minato.isEvalExpr)(rawKey) ? this.encode(key, true, true) : this.escape(rawKey, "json"));
      return notStr ? this.logicalNot(res) : res;
    }
  }
...
var SQLiteDriver = class extends import_minato2.Driver {
  ...
  async prepare(table, dropKeys) {
    const columns = this._all(`PRAGMA table_info(${(0, import_sql_utils2.escapeId)(table)})`);
    ...
    for (const key in model.fields) {
      if (!import_minato2.Field.available(model.fields[key])) {
    ...
    if (model.primary && !model.autoInc) {
      indexDefs.push(`PRIMARY KEY (${this._joinKeys((0, import_cosmokit2.makeArray)(model.primary))})`);
    }
    if (model.unique) {
      indexDefs.push(...model.unique.map((keys) => `UNIQUE (${this._joinKeys((0, import_cosmokit2.makeArray)(keys))})`));
    }
    if (model.foreign) {
    ...
    if (!columns.length) {
      ...
      this._run(`CREATE TABLE ${(0, import_sql_utils2.escapeId)(table)} (${[...columnDefs, ...indexDefs].join(", ")})`);
      ...
      this._run(`CREATE TABLE ${(0, import_sql_utils2.escapeId)(temp)} (${[...columnDefs, ...indexDefs].join(", ")})`);
      try {
        this._run(`INSERT INTO ${(0, import_sql_utils2.escapeId)(temp)} SELECT ${fields} FROM ${(0, import_sql_utils2.escapeId)(table)}`);
        this._run(`DROP TABLE ${(0, import_sql_utils2.escapeId)(table)}`);
      } catch (error) {
        this._run(`DROP TABLE ${(0, import_sql_utils2.escapeId)(temp)}`);
        throw error;
      }
      this._run(`ALTER TABLE ${(0, import_sql_utils2.escapeId)(temp)} RENAME TO ${(0, import_sql_utils2.escapeId)(table)}`);
    } else if (alter.length) {
      this.logger.info("auto updating table %c", table);
      for (const def of alter) {
        this._run(`ALTER TABLE ${(0, import_sql_utils2.escapeId)(table)} ${def}`);
      }
    }
    if (dropKeys) return;
    dropKeys = [];
  ...
  async start() {
    ...
    if (this.path !== ":memory:") {
      this.path = (0, import_node_path.resolve)(this.ctx.baseDir, this.path);
    }
    const isBrowser = process.env.KOISHI_ENV === "browser";
    const sqlite = await (0, import_sql.default)({
      locateFile: /* @__PURE__ */ __name((file) => process.env.KOISHI_BASE ? process.env.KOISHI_BASE + "/" + file : isBrowser ? "/modules/@koishijs/plugin-database-sqlite/" + file : (0, import_node_module.createRequire)(import_meta.url || (0, import_node_url.pathToFileURL)(__filename).href).resolve("@minatojs/sql.js/dist/" + file), "locateFile")
    });
    if (this.path !== ":memory:") {
      const dir = (0, import_node_path.resolve)(this.path, "..");
      try {
        await (0, import_promises.access)(dir);
      } catch {
        throw new Error(`The database directory '${(0, import_node_path.resolve)(this.path, "..")}' is not accessible. You may have to create it first.`);
      }
    }
    if (!isBrowser || this.path === ":memory:") {
      this.db = new sqlite.Database(this.path);
    } else {
      const buffer = await (0, import_promises.readFile)(this.path).catch(() => null);
    ...
    this.define({
      ...
      dump: /* @__PURE__ */ __name((value) => (0, import_cosmokit2.isNullable)(value) ? value : +new Date(value), "dump"),
      load: /* @__PURE__ */ __name((value) => (0, import_cosmokit2.isNullable)(value) ? value : new Date(Number(value)), "load")
    });
    this.define({
      types: ["binary"],
      dump: /* @__PURE__ */ __name((value) => (0, import_cosmokit2.isNullable)(value) ? value : new Uint8Array(value), "dump"),
      load: /* @__PURE__ */ __name((value) => (0, import_cosmokit2.isNullable)(value) ? value : import_cosmokit2.Binary.fromSource(value), "load")
    });
    this.define({
      types: ["primary", ...import_minato2.Field.number],
      dump: /* @__PURE__ */ __name((value) => value, "dump"),
      load: /* @__PURE__ */ __name((value) => (0, import_cosmokit2.isNullable)(value) ? value : Number(value), "load")
    });
  }
  _joinKeys(keys) {
  ...
  _export() {
    ...
    return (0, import_promises.writeFile)(this.path, data);
  }
  ...
  async stats() {
    ...
    tableNames.forEach((tbl) => {
      stats.tables[tbl.name] = this._get(`SELECT COUNT(*) as count FROM ${(0, import_sql_utils2.escapeId)(tbl.name)};`);
  ...
  async remove(sel) {
    ...
    if (filter === "0") return {};
    const result = this._run(`DELETE FROM ${(0, import_sql_utils2.escapeId)(table)} WHERE ${filter}`, [], () => this._get(`SELECT changes() AS count`));
  ...
  _update(sel, indexFields, updateFields, update, data) {
    ...
    const builder = new SQLiteBuilder(this, tables);
    (0, import_minato2.executeUpdate)(data, update, ref);
    ...
    const filter = builder.parseQuery(query);
    this._run(`UPDATE ${(0, import_sql_utils2.escapeId)(table)} SET ${assignment} WHERE ${filter}`, updateFields.map((key) => row[key] ?? null));
  }
  async set(sel, update) {
    ...
    if (query.$expr || (0, import_minato2.hasSubquery)(sel.query) || Object.values(update).some((x) => (0, import_minato2.hasSubquery)(x))) {
      const sel2 = this.database.select(table, query);
      sel2.tables[sel.ref] = sel2.tables[sel2.ref];
      delete sel2.tables[sel2.ref];
      sel2.ref = sel.ref;
      const project = (0, import_cosmokit2.mapValues)(update, (value, key) => () => (0, import_minato2.isEvalExpr)(value) ? value : import_minato2.Eval.literal(value, model.getType(key)));
      const rawUpsert = await sel2.project({
        ...project,
        // do not touch sel2.row since it is not patched
        ...Object.fromEntries(primaryFields.map((x) => [x, () => (0, import_minato2.Eval)("", [sel.ref, x], sel2.model.getType(x))]))
      }).execute();
      const upsert = rawUpsert.map((row) => ({
        ...(0, import_cosmokit2.mapValues)(update, (_, key) => (0, import_minato2.getCell)(row, key)),
        ...Object.fromEntries(primaryFields.map((x) => [x, (0, import_minato2.getCell)(row, x)]))
      }));
  ...
  _create(table, data) {
    ...
    const sql = `INSERT INTO ${(0, import_sql_utils2.escapeId)(table)} (${this._joinKeys(keys)}) VALUES (${Array(keys.length).fill("?").join(", ")})`;
    ...
    for (let i = 0; i < data.length; i += step) {
      ...
      for (const item of chunk) {
        const row = results.find((row2) => {
          ...
          return keys.every((key) => (0, import_cosmokit2.deepEqual)(row2[key], item[key], true));
        });
        if (row) {
          this._update(sel, keys, updateFields, item, row);
          result.matched++;
        } else {
          this._create(table, (0, import_minato2.executeUpdate)(model.create(), item, ref));
          result.inserted++;
        }
      }
    }
  ...
  async createIndex(table, index) {
    ...
    await this._run(`create ${index.unique ? "UNIQUE" : ""} index ${(0, import_sql_utils2.escapeId)(name)} ON ${(0, import_sql_utils2.escapeId)(table)} (${keyFields})`);
  }
  async dropIndex(table, name) {
    await this._run(`DROP INDEX ${(0, import_sql_utils2.escapeId)(name)}`);
  }
  _parseIndexDef(def) {
    if (!def) return {};
    try {
