...
function apply(ctx, config) {
  ...
  function findCommand(target, session) {
    ...
    const data = ctx.i18n.find("commands.(name).shortcuts.(variant)", target).map((item) => ({ ...item, command: $.resolve(item.data.name, session) })).filter((item) => item.command?.match(session));
