...
const logger = new koishi_1.Logger('puppeteer');
class Puppeteer extends koishi_1.Service {
    constructor(ctx, config) {
        super(ctx, 'puppeteer');
        this.config = config;
        this.page = () => this.browser.newPage();
        this.svg = (options) => new svg_1.SVG(options);
        this.render = async (content, callback) => {
            const page = await this.page();
            if (content)
                await page.setContent(content);
            callback || (callback = async (_, next) => page.$('body').then(next));
    ...
    async start() {
        ...
        if (!executablePath) {
            logger.info('chrome executable found at %c', executablePath = (0, puppeteer_finder_1.default)());
        }
        ...
        this.ctx.component('html', async (attrs, children, session) => {
            ...
            try {
                ...
                if (attrs.src) {
                    ...
                    await page.goto('file:///' + (0, path_1.resolve)(__dirname, '../index.html'));
            ...
            <head>${koishi_1.h.unescape(head.join(''))}</head>
            <body style="${bodyStyle}">${koishi_1.h.unescape(content)}</body>
          </html>`);
                }
    ...
    Puppeteer.Config = koishi_1.Schema.intersect([
        koishi_1.Schema.object({
            ...
            args: koishi_1.Schema.array(String)
                ...
                .default(((_a = process.getuid) === null || _a === void 0 ? void 0 : _a.call(process)) === 0 ? ['--no-sandbox'] : []),
        }).description('启动设置'),
        koishi_1.Schema.object({
